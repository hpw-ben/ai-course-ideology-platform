  ...
  <div class="science-container">
    ...
    <div class="main-content">
      <div class="page-header">
        <div class="page-title">{{ item.title || moduleLabel }}</div>
        <el-button type="text" class="back-btn" @click="router.back()">返回</el-button>
      </div>

      <div class="content-card" v-loading="loading">
        <video v-if="item.contentType !== 'ARTICLE' && item.coverImage" class="video-player" controls preload="metadata">
          <source :src="item.coverImage" type="video/mp4" />
        </video>
        <div v-else-if="item.contentType === 'ARTICLE'" class="article-content" v-html="safeArticleHtml"></div>
...
export default {
  name: 'TeacherScienceModuleWatch',
  setup() {
    ...
    const moduleKey = computed(() => route.params.module || route.meta.module)
    const moduleLabel = computed(() => moduleMap[moduleKey.value] || '')
    ...
    const safeArticleHtml = computed(() => sanitizeHtml(item.value.articleContent || ''))
    ...
    const load = async () => {
      ...
      try {
        ...
        if (res.data.code === 200 && res.data.data) {
          if (res.data.data.module !== moduleKey.value) {
...
.article-content { padding: 12px; line-height: 1.8; color: #333; white-space: pre-wrap; }
</style>
