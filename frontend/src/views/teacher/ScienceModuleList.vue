  ...
  <div class="science-container">
    ...
    <div class="main-content">
      <div class="page-header">
        <div class="page-title">{{ moduleLabel }}</div>
...
export default {
  ...
  setup() {
    ...
    const load = async () => {
      ...
      loading.value = true
      try {
        const res = await getActiveScienceItems(moduleKey.value)
    ...
    const openWatch = (item) => {
      ...
      router.push(`/teacher/science/${moduleKey.value}/${item.id}`)
    }
...
.title-row:hover { border-color: #c41230; background: #fff5f6; }
.title-text { font-size: 14px; font-weight: 700; color: #222; }
