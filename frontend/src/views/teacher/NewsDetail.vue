          ...
          <div class="comment-item" v-for="comment in comments" :key="comment.id">
            ...
            <div class="comment-content">
              ...
              <div v-if="replyingTo === comment.id" class="reply-input">
                <el-input v-model="replyContent" type="textarea" :rows="2" :placeholder="'回复 ' + comment.userRealName + '...'" />
              ...
              <div v-if="comment.replies && comment.replies.length > 0" class="replies-list">
                <div class="reply-item" v-for="reply in comment.replies" :key="reply.id">
                  <div class="reply-avatar">
                    ...
                    <div v-else class="reply-avatar-placeholder">{{ reply.userRealName ? reply.userRealName[0] : 'U' }}</div>
                  </div>
                  <div class="reply-content">
                    <div class="reply-header">
                      <span class="reply-name">{{ reply.userRealName || reply.userName }}</span>
                      <span class="reply-type">{{ reply.userType === 'STUDENT' ? '学生' : '教师' }}</span>
                      <span v-if="reply.replyToUserRealName" class="reply-to">回复 <span class="reply-to-name">{{ reply.replyToUserRealName }}</span></span>
                    </div>
                    <div class="reply-text">{{ reply.content }}</div>
                    <div v-if="reply.imageUrl" style="margin-top: 6px;">
                      <img :src="reply.imageUrl" style="max-width: 100%; max-height: 220px; border-radius: 6px;" />
                    </div>
                    <div class="reply-footer">
                      <span class="reply-time">{{ formatTime(reply.createdAt) }}</span>
                      <el-button type="text" size="small" :disabled="!user.id" @click="toggleLike(reply)">
                        {{ reply.liked ? '已赞' : '点赞' }} ({{ reply.likeCount || 0 }})
                      </el-button>
                      <el-button type="text" size="small" @click="showReplyToReply(comment, reply)">回复</el-button>
                    </div>
                  </div>
                </div>
                
                <!-- 回复子评论的输入框 -->
                <div v-if="replyingToReply && replyingToReply.parentId === comment.id" class="reply-input">
    ...
    const loadNews = async () => {
      try {
        ...
        if (res.data.code === 200) {
          ...
          router.push(isFromThought.value ? '/teacher/thought' : '/teacher/home')
        }
      } catch (e) {
...
<style scoped>
.news-detail-container { min-height: 100vh; background: #f5f5f5; }
.navbar { background: #c41230; position: fixed; top: 0; left: 0; right: 0; z-index: 100; box-shadow: 0 2px 8px rgba(0,0,0,0.15); }
.nav-content { max-width: 1200px; margin: 0 auto; display: flex; align-items: center; justify-content: space-between; padding: 0 20px; height: 60px; }
.logo { font-size: 20px; font-weight: bold; color: #fff; }
.main-content { padding-top: 80px; max-width: 900px; margin: 0 auto; padding-bottom: 40px; }
.news-content { background: #fff; padding: 40px; border-radius: 8px; margin-bottom: 20px; }
.news-title { font-size: 32px; color: #333; margin: 0 0 20px 0; line-height: 1.4; }
.news-meta { display: flex; gap: 20px; color: #999; font-size: 14px; padding-bottom: 20px; border-bottom: 1px solid #eee; margin-bottom: 20px; }
.news-summary { font-size: 16px; color: #666; line-height: 1.8; margin-bottom: 20px; padding: 15px; background: #f9f9f9; border-left: 4px solid #c41230; }
.news-body { font-size: 16px; color: #333; line-height: 2; white-space: pre-wrap; overflow-wrap: anywhere; word-break: break-word; }
.comment-section { background: #fff; padding: 30px; border-radius: 8px; }
.comment-header { border-bottom: 2px solid #c41230; padding-bottom: 15px; margin-bottom: 20px; }
.comment-header h3 { margin: 0; color: #333; }
.comment-input { margin-bottom: 30px; }
.comment-list { display: flex; flex-direction: column; gap: 20px; }
.comment-item { display: flex; gap: 15px; padding: 15px; background: #f9f9f9; border-radius: 8px; }
...
.avatar-placeholder { width: 100%; height: 100%; border-radius: 50%; background: #c41230; color: #fff; display: flex; align-items: center; justify-content: center; font-size: 18px; font-weight: bold; }
.comment-content { flex: 1; }
.comment-user { display: flex; align-items: center; gap: 10px; margin-bottom: 8px; }
.user-name { font-weight: bold; color: #333; }
.user-type { font-size: 12px; color: #999; padding: 2px 8px; background: #fff; border-radius: 4px; }
.comment-text { color: #666; line-height: 1.6; margin-bottom: 8px; }
.comment-footer { display: flex; align-items: center; gap: 15px; }
...
.replies-list { margin-top: 10px; padding-left: 50px; display: flex; flex-direction: column; gap: 10px; }
.reply-item { display: flex; gap: 10px; padding: 10px; background: #fff; border-radius: 4px; border-left: 3px solid #c41230; }
.reply-avatar { width: 32px; height: 32px; flex-shrink: 0; }
.reply-avatar img { width: 100%; height: 100%; border-radius: 50%; object-fit: cover; }
.reply-avatar-placeholder { width: 100%; height: 100%; border-radius: 50%; background: #c41230; color: #fff; display: flex; align-items: center; justify-content: center; font-size: 14px; font-weight: bold; }
...
.reply-type { font-size: 11px; color: #999; padding: 1px 6px; background: #f5f5f5; border-radius: 3px; }
.reply-to { color: #909399; font-size: 12px; }
.reply-to-name { color: #c41230; }
.reply-text { color: #666; font-size: 13px; line-height: 1.5; margin-bottom: 6px; }
