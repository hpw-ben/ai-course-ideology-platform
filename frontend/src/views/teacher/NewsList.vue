  ...
  <div class="news-list-container">
    <div class="navbar">
      <div class="nav-content">
        ...
        <el-menu mode="horizontal" default-active="home" class="nav-menu">
...
export default {
  ...
  setup() {
    ...
    const loadList = async () => {
      loading.value = true
      try {
        const res = await getPublishedNews(category.value || undefined)
    ...
    const openNews = (id) => {
      if (!id) return
      router.push({ path: `/teacher/news/${id}`, query: { from: 'news-list' } })
    }
...
<style scoped>
.news-list-container { min-height: 100vh; background: #f5f5f5; }
...
.content-area { max-width: none; width: 100%; margin: 0; padding: 18px 32px; }
...
.list-card { background: #fff; border-radius: 10px; padding: 18px 22px; min-width: 0; width: 100%; }
.list { display: flex; flex-direction: column; gap: 14px; }
.row { display: flex; align-items: center; gap: 16px; min-width: 0; padding: 6px 0; }
...
.date { flex: none; color: #999; font-size: 14px; }
