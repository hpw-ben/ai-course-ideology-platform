  ...
  <div class="task-container">
    ...
    <div class="content">
      ...
      <div class="material-card" v-if="task.materials && task.materials.length > 0">
        <h3>关联素材（{{ task.materials.length }}个）</h3>
      ...
      <div class="info-card" v-if="task.newsList && task.newsList.length > 0">
        <h3>关联新闻（{{ task.newsList.length }}条）</h3>
        <div>
      ...
      <!-- 兼容旧数据：单素材 -->
      <div class="material-card" v-else-if="task.material">
        <h3>关联素材</h3>
        <div class="material-info">
          ...
          <div v-if="task.material.type === 'IMAGE' && task.material.fileUrl">
            <img :src="task.material.fileUrl" class="material-img" />
          </div>
          <div v-if="task.material.type === 'VIDEO' && task.material.fileUrl">
            <video :src="task.material.fileUrl" controls class="material-video"></video>
          </div>
          <div v-if="task.material.type === 'ARTICLE'" class="article-content">
      ...
      <!-- 教师发表评论 -->
      <div class="comment-form-card" v-if="task.status !== 'ENDED'">
        <h3>发表评论</h3>
...
import { getTaskById, getTaskCommentsWithUser, addTaskComment, deleteTaskComment, togglePinTaskComment, toggleTaskCommentLike } from '../../api/task'
...
export default {
  ...
  setup() {
    ...
    const loadComments = async () => {
      try {
        const res = await getTaskCommentsWithUser(route.params.id, user.value.id, user.value.role)
        if (res.data.code === 200) {
    ...
    const copyCode = () => {
      navigator.clipboard.writeText(task.value.code)
      ElMessage.success('任务码已复制')
    }
    ...
    const removeReply = async (r) => {
      try {
        const res = await deleteTaskComment(r.id)
        if (res.data.code === 200) {
          ElMessage.success('删除成功')
          loadComments()
        }
    ...
    const togglePin = async (c) => {
      try {
        const res = await togglePinTaskComment(c.id, task.value.id)
        if (res.data.code === 200) {
    ...
    return { 
      ...
      removeComment, removeReply, togglePin, Back 
    }
  }
}
</script>
...
<style scoped>
.task-container { padding: 20px; max-width: 900px; margin: 0 auto; }
