...
export default {
  ...
  setup() {
    ...
    const openNews = (id) => {
      if (!id) return
      router.push({ path: `/student/news/${id}`, query: { from: 'thought' } })
    }

    const goSection = (sec) => {
      router.push({ path: '/student/thought/list', query: { category: sec } })
    }
